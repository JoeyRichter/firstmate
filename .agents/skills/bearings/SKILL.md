...
- Any Captain's Call item or fleet row MAY carry an optional `detail_url` so the captain can chase down more context without parsing chat history: a scout report, a plan, a diagram, a local file, or an external site. Set it to an `https://` URL or an absolute local filesystem path (the board renders the path as a `file://` link that opens directly); the board derives a short label and refuses anything that is neither. Only add it when a concrete pointer genuinely helps, and omit it or pass null otherwise. `bin/fm-bearings-board.sh`'s header owns the accepted formats.
