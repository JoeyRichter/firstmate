...
# and its canonical root, and a seeded secondmate home, are trusted so the agent
# reaches its brief or charter with no human, and every out-of-scope path is
# REFUSED rather than warned about or quietly skipped.
set -u
...
test_fresh_worktree_is_trusted() {
  ...
  # Claude's per-project lookup keys on the canonical git root, the main working
  # tree a linked worktree's .git file points at, so the worktree entry alone
  # leaves a worker parked on the gated-grants variant of the dialog.
...
# The canonical root Claude keys on is the repository's MAIN working tree, which
# is not always the project argument: a linked spawning home (fm-spawn.sh's
# header) is a firstmate home that is itself a linked worktree of the project
# repository, and spawns from it must keep launching. The registered root must
# then be the main checkout, and the home's own path must never be written,
# because Claude does not look there.
...
# canonical root for Claude's lookup to find and nothing safe to guess at.
...
# primary checkout in Claude, and Claude records its per-project state there.
...
# A secondmate home is registered on its own: the pane starts in the home and
# works across it, and a leased-worktree home must NOT drag the firstmate
# repository's main checkout into a standing trust grant as a side effect.
test_secondmate_leased_worktree_home_registers_only_the_home() {
  local rec out home
  rec=$(make_case sm-home-only)
  read_case "$rec"
  home="$CASE_DIR/sm-home"
  seed_secondmate_home "$home" sm-home-only worktree
  out=$(run_home_trust "$CONFIG" "$home" sm-home-only)
  expect_code 0 $? "a seeded leased-worktree home must be trusted: $out"
  assert_trusted "$CONFIG/.claude.json" "$home" "the secondmate home was not recorded as trusted"
  assert_not_trusted "$CONFIG/.claude.json" "$home.src" "the leased home's repository main checkout was trusted as a side effect"
  pass "fm-claude-trust.sh: a leased-worktree secondmate home registers only the home"
}
