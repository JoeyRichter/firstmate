...
- Cleanup still closes only the exact recorded task pane, so on a host running a pane-injecting plugin the companion pane keeps the tab alive and the disposable workspace is retained after its task ends; creation-side identity is fixed, and this retained-workspace case is deferred to the `fm-herdr-adapter-pane-identity-audit` item covering every site that still assumes one close empties its container.
